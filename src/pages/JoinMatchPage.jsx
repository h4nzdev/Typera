...
const JoinMatchPage = () => {
  ...
  const [step, setStep] = useState(1); // 1 = Match Code, 2 = Player Name
  const [code, setCode] = useState('');
  const { setPlayerName, playerName } = useUserStore();
  const [challengerName, setChallengerName] = useState(() => playerName || '');
  ...
  const handleJoinFinal = (e) => {
    ...
    setError('');
    playSound('click');
    const cleanName = challengerName.trim().toUpperCase();
        ...
        <PixelPanel color="#ff007f">
          ...
          <div className="flex items-center justify-between mb-6 border-b-2 pb-3" style={{ borderColor: 'rgba(255,0,127,0.3)' }}>
            <span className="font-[family-name:var(--font-arcade)] text-xs tracking-widest text-pink-400">
              ● STEP {step} OF 2: {step === 1 ? 'MATCH CODE' : 'YOUR NAME'}
            </span>
          ...
          {/* STEP 1: MATCH CODE ENTRY */}
          {step === 1 && (
            <form onSubmit={handleNextToName} className="flex flex-col items-center gap-6">
              ...
              <div className="flex flex-col gap-3 w-full border-t-2 pt-4" style={{ borderColor: 'rgba(255,0,127,0.2)' }}>
                <ArcadeButton type="submit" color="pink" className="w-full py-3">
          ...
          {/* STEP 2: PLAYER NAME ENTRY */}
          {step === 2 && (
            <form onSubmit={handleJoinFinal} className="flex flex-col items-center gap-6">
              {/* Confirmed Match Code Header Badge */}
              <div className="flex items-center justify-between w-full bg-black/60 border border-pink-500/40 px-4 py-2 rounded-xl">
                <span className="font-[family-name:var(--font-arcade)] text-xs text-white/60">MATCH CODE:</span>
                <span className="font-[family-name:var(--font-arcade)] text-base font-bold text-pink-400 tracking-widest">{code}</span>
              ...
              <div className="w-full flex flex-col items-center gap-3">
                <span className="font-[family-name:var(--font-arcade)] text-xs tracking-widest text-pink-300">ENTER YOUR PLAYER NAME</span>
                <input
                  type="text"
                  maxLength={5}
                  value={challengerName}
                  onChange={(e) => { setChallengerName(e.target.value.toUpperCase()); setError(''); }}
                  placeholder="PLAYER"
                  autoFocus
                  className="w-full bg-black border-2 border-[var(--color-neon-pink)] px-4 py-3 text-center font-[family-name:var(--font-arcade)] text-3xl tracking-widest text-pink-300 outline-none uppercase"
