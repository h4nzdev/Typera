...
const CreateMatchPage = () => {
  ...
  const { initMatch, leaveMatch, status, category, setCategory, gameMode, setGameMode, allowDebuffs, setAllowDebuffs, allowBossWord, setAllowBossWord } = useMatchStore();
  ...
  useEffect(() => {
    ...
    const hostDefault = playerName || 'PLAYER 1';
    setPlayerName(hostDefault);
    ...
    const setupMatch = async () => {
      ...
      while (attempts < 3) {
        ...
        try {
          ...
          console.warn("Failed to create match with code", code, error);
          attempts++;
        }
      }
          ...
          <div className="flex flex-col items-center gap-6">
            {isCreating ? (
               <div className="flex flex-col items-center gap-4 py-8">
                 <div className="w-8 h-8 border-4 border-[var(--color-neon-blue)] border-t-transparent rounded-full animate-spin" />
                 <span className="font-[family-name:var(--font-arcade)] text-sm tracking-widest text-white/60">
                    GENERATING ROOM{'.'?.repeat(dotCount) + ' '.repeat(3 - dotCount)}
                 </span>
               </div>
            ) : createError ? (
               <div className="flex flex-col items-center gap-4 py-8">
                  <span className="font-[family-name:var(--font-arcade)] text-sm tracking-widest text-red-500 animate-pulse text-center">
                    {createError}
                  </span>
                  <ArcadeButton type="button" color="cyan" onClick={() => window.location.reload()} className="w-full text-xs py-2">
                    RETRY
                  </ArcadeButton>
               </div>
            ) : (
              <>
                ...
                <div className="w-full flex flex-col gap-4 border-t-2 pt-4" style={{ borderColor: `${modeColor}30` }}>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap justify-center gap-2">
                        {['all', 'common', 'it', 'gaming', 'tech', 'fun'].map(c => (
                          <button key={c} type="button" onClick={() => { playSound('click'); setCategory(c); }}
                    ...
                    <div>
                      <div className="font-[family-name:var(--font-arcade)] text-xs tracking-widest text-white/40 mb-2 text-center">ALLOW BOSS WORD</div>
                      <div className="flex gap-2">
                        {[true, false].map(val => (
                          <button key={val ? 'on' : 'off'} type="button" onClick={() => { playSound('click'); setAllowBossWord(val); }}
                            className="flex-1 py-2 font-[family-name:var(--font-arcade)] text-[10px] sm:text-xs tracking-widest transition-all border-2"
                            style={{
                              borderColor: allowBossWord === val ? (val ? '#00f3ff' : '#ff003c') : '#333',
                              color: allowBossWord === val ? (val ? '#00f3ff' : '#ff003c') : '#555',
                              background: allowBossWord === val ? (val ? 'rgba(0,243,255,0.08)' : 'rgba(255,0,60,0.08)') : 'transparent',
                            }}>
                            {val ? 'ON' : 'OFF'}
                          </button>
                        ))}
